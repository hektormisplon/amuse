...
import {
    Blog, Category, Museum, Post, Tour, User,
} from './schemas';
...
class Seeder {
    ...
    museumCreate = async (shortName, name, coords) => {
        ...
        try {
            ...
            logger.log({
                level: 'info',
                message: `An error occurred when creating a museum: ${err}!`,
            });
        }
    };
    ...
    createMusea = async () => {
        await Promise.all([
            (async () => this.museumCreate(faker.lorem.sentence(), faker.lorem.sentence(), [
                { lng: 0, lat: 0 },
            ]))(),
            (async () => this.museumCreate(faker.lorem.sentence(), faker.lorem.sentence(), [
                { lng: 0, lat: 0 },
    ...
    seed = async () => {
        ...
        this.tours = await Tour.estimatedDocumentCount()
            .exec()
            .then(async (count) => {
                if (count === 0) {
        ...
        this.musea = await Museum.estimatedDocumentCount()
            .exec()
            .then(async (count) => {
                if (count === 0) {
